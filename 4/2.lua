...
local rules = {
    byr = {type = "year", min = 1920, max = 2002},
    iyr = {type = "year", min = 2010, max = 2020},
    eyr = {type = "year", min = 2020, max = 2030},
    hgt = {
        type = "length",
        cm = {min = 150, max = 193},
        ["in"] = {min = 59, max = 76}
...
local function setNextPassport()
    ...
    if Animate.timescale >= 10 then
        ...
        local valid = pp:isValid(nil, rules)
        ...
        Animate.new {
            duration = valid and 0.1 or 0.3,
                ...
                if #attributes > 0 then
                    checkAttribute()
                elseif pp:isValid(nil, rules) then
                    setStamp(true)
                end
            end
        }:start()
